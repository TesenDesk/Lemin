...
/*   Updated: 2019/10/09 14:00:25 by mstygg           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char			*manage_str_gnl(t_avl_t *r, char *ptr, char *bf, size_t size)
{
	char			*new;
	size_t			n_s;
...
int					get_next_line(const int fd, char **line)
{
	...
	if (!(*line = manage_str_gnl(tmp, check, bf, count)) && count)
		return (-1);
	free(bf);
	if (!(*line) && !count)
